...
class Test(unittest.TestCase):
	...
	def test_transfactors(self):
		...
		self.assertEqual('[AC][AC]CCA[GC]C', read_factors.dt_trans_factors['Ace2p'].forward)
		self.assertEqual('G[GC]TGG[GT][GT]', read_factors.dt_trans_factors['Ace2p'].reverse)
	...
	def test_get_diff_between_two_seq(self):
		...
		self.assertEqual(3, len(dt_out_lost_ref))
		self.assertEqual(1, len(dt_out_lost_ref['Cta8p_2']))
		self.assertEqual(1, len(dt_out_lost_ref['Mrr1p']))
		self.assertEqual(1, len(dt_out_lost_ref['Mrr1p_1']))
		self.assertEqual(1627, dt_out_lost_ref['Cta8p_2'][0].position_start)
		self.assertEqual("TTCACCACTTTTC", dt_out_lost_ref['Cta8p_2'][0].sequence_found)
		...
		self.assertEqual(7, len(dt_out_gain_change_1))
		self.assertEqual(3, len(dt_out_lost_ref))
		...
		result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)), "files/results/" + Constants.FILE_NAME_OUT_GAIN)
		self.assertTrue(os.path.exists(result_file_name))
		self.assertTrue(self.utils.test_equal_files(result_file_name, os.path.join(out_path, Constants.FILE_NAME_OUT_GAIN)))
		result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)), "files/results/" + Constants.FILE_NAME_OUT_LOST)
		self.assertTrue(os.path.exists(result_file_name))
	...
	def test_get_diff_between_two_samples(self):
		...
		result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)), "files/results_2/" + Constants.FILE_NAME_OUT_GAIN)
		self.assertTrue(os.path.exists(result_file_name))
		self.assertTrue(self.utils.test_equal_files(result_file_name, os.path.join(out_path, Constants.FILE_NAME_OUT_GAIN)))
